...
canonical_aas = {'CYS': 'C', 'ASP': 'D', 'SER': 'S', 'GLN': 'Q', 'LYS': 'K', 'ILE': 'I', 'PRO': 'P', 'THR': 'T', 'PHE': 'F', 'ASN': 'N', 'GLY': 'G', 'HIS': 'H', 'LEU': 'L', 'ARG': 'R', 'TRP': 'W', 'ALA': 'A', 'VAL': 'V', 'GLU': 'E', 'TYR': 'Y', 'MET': 'M'}

antibody_id = input("Enter name of antibody:\n")
antibody_id = antibody_id.strip()
H_chain = input("Chain ID of heavy chain:")
H_chain = H_chain.strip()
L_chain = input("Chain ID of light chain:")
L_chain = L_chain.strip()
Ag_chain = input("Chain ID of antigen chain:")
...
for model in docked_models:
	...
	for residue in epi_res:
		...
		if res_name in canonical_aas.keys():
