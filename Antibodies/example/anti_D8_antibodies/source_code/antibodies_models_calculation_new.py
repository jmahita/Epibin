...
def number_of_models(threshold, file_name):
	...
	antibody_pair = file_name.split("_")
	lines1 = csv.reader(input_file)
	threshold = threshold.strip()
	count_row = 0
	total_rows = 0
	print(antibody_pair[0])
	print(antibody_pair[1])
...
def averaged(threshold, file_name):
	...
	input_file_2 = transpose_file+"_T.csv"
	antibody_pair = file_name.split("_")
	normalized_value_row = number_of_models(threshold, file_name)
	normalized_value_column = number_of_models(threshold, input_file_2)
	...
	return(antibody_pair[0], antibody_pair[1], epibin_score)
	input_file_1.close()
	input_file_2.close()
...
output_file = open("antibody_epibin_EOS_"+user_input_1+".csv", "w")
csv_writer = csv.writer(output_file)
row1 = []
row1.append("Antibody 1")
row1.append("Antibody 2")
